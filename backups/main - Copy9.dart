...
// -----------------------------------------------------------------------------
// Web changes:
// - Exporter is platform-aware (Web: Blob download; Android/iOS: Share sheet).
// - HistoryPage CSV/XLS export now produces multi-sheet Excel (.xls XML).
//
...
// - Log highlights only the single latest current-IN row per name|tag.
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  alignment: WrapAlignment.center,
                  children: [
                    for (final c in ["BLUE", "GREEN", "RED", "WHITE"])
                      ElevatedButton(
...
class _HistoryPageState extends State<HistoryPage> {
  ...
  // Core: get logs for a specific tab, marking ONLY the single latest IN row per diver as current.
  List<Map> _logsForTab(String filterTab) {
    ...
    // Latest event and latest IN timestamp per diver across ALL colors
    final Map<String, Map> latestEventByKey = {};
    final Map<String, String> latestInDtByKey = {};
    for (final log in allNewestFirst) {
      final key = "${log['name']}|${log['tag']}";
      latestEventByKey.putIfAbsent(key, () => log); // first seen is latest
    ...
    // Apply color filter (not for ALL or CHECKED-IN)
    ...
    // Compute dive durations for OUT rows within visible list
    ...
    // Mark only the single latest IN row per diver as current
    ...
    // Sort with currently-IN first, then by datetime desc
    logsList.sort((a, b) {
  ...
  // Adapter used by the UI (current tab)
  ...
  // New: multi-worksheet Excel 97–2003 XML (.xls)
  String _buildXlsXml() {
    ...
    // Helper to add a logs sheet for a tab
