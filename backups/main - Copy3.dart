...
// ===================== AQUACOULISSE SCREEN (UPDATED CENTERING) =============
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          TopAlert(currentlyIn: currentlyIn),
          // Back button pinned truly top-left
          Positioned(
          ...
          // Centered content: we use Align center; column minimal; button row center in screen
          Align(
...
class _HistoryPageState extends State<HistoryPage> {
  ...
  void initState() {
    ...
    _logsSub = logsBox.watch().listen((_) {
      if (mounted) setState(() {}); // live update
    });
  }
  ...
  Future<void> _exportCSV({required bool asExcel}) async {
    final logs = getLogs();
    ...
    final fileName =
        "dives_${DateTime.now().millisecondsSinceEpoch}.${asExcel ? "xlsx" : "csv"}";
    ...
    await Share.shareXFiles(
      [XFile(file.path)],
      text: asExcel
          ? "Dive History Export (Excel compatible)"
          : "Dive History CSV Export",
      subject: "Dive History Export",
    );
  }
                  ...
                  Row(
                    children: [
                      ...
                      PopupMenuButton<String>(
                        onSelected: (val) {
                          if (val == 'csv') _exportCSV(asExcel: false);
                          if (val == 'xlsx') _exportCSV(asExcel: true);
                        },
                        itemBuilder: (_) => const [
                          PopupMenuItem(
                            value: 'csv',
                          ...
                          PopupMenuItem(
                            value: 'xlsx',
                            child: Text('Export Excel (.xlsx) & Share'),
                          ),
                        ],
                        child: ElevatedButton(
                          style: ElevatedButton.styleFrom(
